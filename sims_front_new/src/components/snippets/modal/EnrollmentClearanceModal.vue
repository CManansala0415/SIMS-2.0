...
import { getCurriculumSubject, getEnrollment, getMilestone, addMilestone, updateEnrollment, updateMilestone, getCommandUpdateCurriculum, getEnrollmentSchedule, getLaunchChecker } from "../../Fetchers.js";
...
onMounted(async () => {
    ...
    try {
        ...
        getEnrollment(studentData.value.per_id).then((results) => {
            ...
            let sem  = enrolleeData.value[0].quar_id
            ...
            getCommandUpdateCurriculum(prog, grad, cour).then((results) => {
                ...
                getCurriculumSubject(curr, 0, 0).then((results) => {
                    ...
                    getMilestone(studentData.value.enr_id).then((results2) => {
                        ...
                        getLaunchChecker(prog, sem, cour, grad, curr, enr_section.value).then(async (results3) => {
                            getEnrollmentSchedule(curr, prog, grad, cour, enr_section.value, results3.ln_id).then((results4) => {
                                scheduleData.value = results4.data

                                // console.log(scheduleData.value)
                                // console.log(milestone.value)
                                let x = attachFaculty(milestone.value,scheduleData.value)
                                milestone.value = x
...
const attachFaculty = (primary, schedules) =>{
    return primary.map(subject => {
        ...
        schedules.forEach(schedule => {
            ...
            days.forEach(day => {
                if (schedule[`sched_${day}_code`] === subject.subj_code) {

                    const fname = schedule[`${day}_faculty_firstname`];
                    const mname = schedule[`${day}_faculty_middlename`];
                    const lname = schedule[`${day}_faculty_lastname`];
                    const suffix = schedule[`${day}_faculty_suffixname`];
