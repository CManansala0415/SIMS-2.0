            ...
            */
            ...
            foreach ($milestonedata as $ms) {
                ...
                if ($template) { 
                    ...
                    $lab_amount += ((float) ($template->tuitemp_lab_price ?? 0) * (float) ($computedLab ?? 0));
        ...
        try {
            ...
            foreach ($templatePricesData as $price) {
                ...
                ->insert([
                    ...
                    'soa_headerid' => $price->act_headerid ?? 'milestone-'.$firstMilestone->enr_id, // if from template, use headerid, else use milestone id with prefix
                    'soa_curriculum' => $firstMilestone->enr_curriculum ?? null,
                    'soa_quarter' => $firstMilestone->enr_quarter ?? null,
                    'soa_program' => $firstMilestone->enr_program ?? null,
                    'soa_course' => $firstMilestone->enr_course ?? null,
                    'soa_gradelvl' => $firstMilestone->enr_gradelvl ?? null,
                    'soa_dateenrolled' => $firstMilestone->enr_dateenrolled ?? null,
                    'soa_enrolledby' => $firstMilestone->enr_enrby ?? null,
                    'soa_section' => $firstMilestone->enr_section ?? null,
                    'soa_actid' => $price->tuitemp_id ?? null,
                    'soa_acsid' => $settlementData->acs_id ?? null,
                    'soa_itemid' => $price->tuitemp_itemid ?? null,
                    'soa_subjid' => $price->tuitemp_subjid ?? null,
                    'soa_custid' => $price->tuitemp_custid ?? null,
                    'soa_custype' => $price->tuitemp_custype ?? null,
                    'soa_desc' => $price->tuitemp_desc ?? null,
                    'soa_price' => $price->tuitemp_price ?? null,
                    'soa_quantity' => $price->tuitemp_quantity ?? null,
                    'soa_disc_type' => $price->tuitemp_disc_type ?? 0,
                    'soa_lec_price' => $price->tuitemp_lec_price ?? null,
                    'soa_lab_price' => $price->tuitemp_lab_price ?? null,
                    'soa_lec' => $price->tuitemp_lec ?? 0,
                    'soa_lab' => $price->tuitemp_lab ?? 0,
                    'soa_subjcode' => $price->tuitemp_subjcode ?? null,
                    'soa_addedby' => $price->tuitemp_updatedby ?? null,
                    'soa_dateadded' => $date,
                ]);
            }
